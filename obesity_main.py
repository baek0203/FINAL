...
class ScaleContinuous(BaseEstimator, TransformerMixin):
    ...
    #######################################################################
def get_weight_level(prediction):
    levels = ["Insufficient Weight", "Normal Weight", "Overweight Level I", "Overweight Level II", "Obesity Type I", "Obesity Type II", "Obesity Type III"]
...
if st.button('제출'):
    ...
    index_list = ['Gender', 'Age', 'Height', 'Weight',
                  'family_history_with_overweight', 'FAVC', 'FCVC', 'NCP', 'CAEC',
                  'CH2O', 'SCC', 'FAF', 'TUE', 'CALC', 'MTRANS']


    # 데이터 프레임으로 변환
    input_df = pd.DataFrame(input_data, columns = index_list)
